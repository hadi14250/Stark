...
import { DEFAULT_CONFIG, type GalleryConfig } from "@/lib/gallery/config";
import {
  CELL_IMAGE,
  PUSH_EASE,
  mirrorDir,
...
export default function PushSlider({
  ...
  /** Shown when the sub-category has no pictures. Already localised. */
  emptyLabel?: string;
  /** Already localised. */
  labels: { prev: string; next: string };
  /** Open the lightbox on picture `index` of the active sub-category. */
  onOpen?: (index: number, el: HTMLButtonElement) => void;
  /** True while the lightbox is open — holds autoplay. */
  paused?: boolean;
}) {
  ...
  // The editor's live text / theme / image-override layers are gone, and the
  // review removed the text they applied to. `effSlides` is kept as the single
  // name everything downstream reads (stage vars, per-layer palette freeze,
  // neighbour preload) rather than threading `slides` through eight call sites.
  const effSlides = slides;
   ...
   * autoplay — pushes up from here, where an actual move is
  ...
  // `paused` is the lightbox: advancing the stage underneath an open preview
  // would swap the picture out from under the reader.
  const autoplayPaused =
    paused || docHidden || (m.autoplay.pauseOnHover && hovering) || atEnd;
