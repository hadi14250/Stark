...
/**
 * Which cells run the WebGL shader (null = DOM fallback instead) — the four
 * pure-photo cells. Each mounts its own short-lived WebGL context for the
 * ~1.2s of a transition (well under the browser's ~16-context limit; the
 * context is reclaimed when the canvas leaves the DOM at transition end).
 ...
 * (see ShaderCrossfade). The cuisine cell is a text+photo hybrid and the
 * rest are text, so they take the effect's DOM fallback (ripple = radial
 * scale-bloom, liquid-warp = directional drift) rather than a shader.
 */
export const CELL_IMAGE: Record<CellId, ((s: Slide) => string) | null> = {
  ...
  cuisine: null, // hybrid: a full-card shader would cover the copy
  intro: null,
  explore: null,
  stay: null,
  cta: null,
